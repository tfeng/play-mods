 ...
 */
public class MongoDbIndexProvider implements IndexProvider {
  ...
  private static final ALogger LOG = Logger.of(MongoDbIndexProvider.class);
  ...
  private volatile Map<String, MongoDbIndexStore> stores = new ConcurrentHashMap<>();
  ...
  private MongoDbIndexStore getStore(String store) {
    String storeName = store + COLLECTION_POSTFIX;
    MongoDbIndexStore indexStore = stores.get(storeName);
    if (indexStore == null) {
      indexStore = new MongoDbIndexStore(mongoDb, storeName);
